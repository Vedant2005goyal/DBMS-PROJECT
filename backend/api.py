...
def auto_close_expired_sessions():
    """Ensure sessions move to Completed once their end time has passed."""
    try:
        now = datetime.now()
        query = """
            ...
            WHERE Status IN ('Scheduled', 'Ongoing')
              AND TIMESTAMP(Session_Date, End_Time) <= %s
        """
        db.execute_query(query, (now,))
    except Exception as auto_close_error:
        print(f"⚠️  Failed to auto-close sessions: {auto_close_error}")

# Import check for face recognition
try:
    import face_recognition
    print("✅ Face recognition module available in API")
except ImportError:
    print("⚠️ Face recognition module not available in API")
...
def get_session_attendance(session_id):
    """Get attendance for a specific session - shows ALL enrolled students with their status"""
    try:
        # First, get the subject_id from the session
        session_query = "SELECT Subject_ID FROM Attendance_Session WHERE Session_ID = %s"
        session_info = db.execute_query(session_query, (session_id,), fetch_one=True)
        ...
        # Get all students enrolled in this subject with their attendance for this session
        # Also calculate overall attendance percentage
        query = """
            SELECT 
                u.User_ID,
                u.Name as name,
                ...
                (
                    SELECT 
                        CASE 
                            WHEN COUNT(DISTINCT s2.Session_ID) = 0 THEN 0
                            ELSE ROUND(
                                (COUNT(DISTINCT CASE WHEN a2.Status IN ('Present', 'Late') THEN s2.Session_ID END) * 100.0) / 
                                COUNT(DISTINCT s2.Session_ID), 
                                2
                            )
                        END
                    FROM Attendance_Session s2
                    LEFT JOIN Attendance a2 ON s2.Session_ID = a2.Session_ID 
                        AND a2.User_ID = u.User_ID
                        AND a2.Status IN ('Present', 'Late')
                    WHERE s2.Subject_ID = %s 
                        AND s2.Status IN ('Completed', 'Ongoing')
                ) as attendance_percentage
            FROM User u
            JOIN Students s ON u.User_ID = s.User_ID
            JOIN Student_Subject ss ON u.User_ID = ss.User_ID
            LEFT JOIN Attendance a ON u.User_ID = a.User_ID AND a.Session_ID = %s
            WHERE ss.Subject_ID = %s
            ORDER BY s.Roll_no, u.Name
        """
        
        attendance = db.execute_query(query, (subject_id, session_id, subject_id), fetch=True) or []
        
        # Convert datetime to string and set default status
        for record in attendance:
            record['time'] = str(record['time']) if record['time'] else None
            record['Date'] = str(record['Date']) if record['Date'] else None
            # If no attendance record, status is Absent
            if not record['Status']:
                record['Status'] = 'Absent'
            # Ensure attendance_percentage is a number
