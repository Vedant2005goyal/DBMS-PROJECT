...
class MarkAttendance:
    def __init__(self):
        ...
        if FACE_RECOGNITION_AVAILABLE:
            ...
            print("⚠️ Face recognition disabled - MarkAttendance will not work without face_recognition module")

    def ensure_db_connection(self):
        """Ensure MySQL connection/cursor are alive."""
        try:
            if not self.mysql_connection.is_connected():
                self.mysql_connection.reconnect(attempts=3, delay=2)
                self.cursor = self.mysql_connection.cursor(dictionary=True)
    ...
    def load_known_faces(self):
        try:
            ...
            if not results:
                print("WARNING: No faces registered in database! Face recognition will not work.")
            ...
            for row in results:
                ...
                try:
                    ...
                    print(f"Error loading face encoding for {Name} (ID: {User_ID}): {e}")
                    continue
    ...
    def mark_attendance_from_frame(self, frame, subject_id, session_id=None):
        ...
        if session_id:
            ...
            try:
                ...
                print(f"⚠️ DB connection dropped while checking session {session_id}: {e}. Reconnecting...")
                self.ensure_db_connection()
                self.cursor.execute(query, (session_id,))
                session = self.cursor.fetchone()

            if not session:
                # Fallback: if specific session not found, try to use any active
                # session for this subject instead of hard failing.
                print(f"❌ Session ID {session_id} not found in database, falling back to active session for subject {subject_id}")
                sessions = self.get_active_session(Subject_ID=subject_id)
                if not sessions:
                    return {'success': False, 'message': f'Session ID {session_id} not found and no active session for this subject'}
                session = sessions[0]
        ...
        if not FACE_RECOGNITION_AVAILABLE:
            return {'success': False, 'message': 'Face recognition is currently unavailable. Please use "Mark Attendance (Manual)" tab to mark attendance manually.'}
        ...
        # 5. If face recognized, verify user is a student before marking attendance
        if User_ID:
            try:
                # Check if user is a student (not faculty)
                student_check = "SELECT User_ID FROM Students WHERE User_ID = %s"
                self.cursor.execute(student_check, (User_ID,))
                is_student = self.cursor.fetchone()
                
                if not is_student:
                    # User is not a student (likely faculty or missing from Students table)
                    user_role_query = "SELECT User_Role FROM User WHERE User_ID = %s"
                    self.cursor.execute(user_role_query, (User_ID,))
                    user_info = self.cursor.fetchone()
                    role = user_info['User_Role'] if user_info else 'Unknown'
                    
                    if role == 'Student':
                        # User has Student role but not in Students table - data inconsistency
                        print(f"❌ {name} (ID: {User_ID}) has Student role but is not registered in Students table.")
                        return {
                            'success': False, 
                            'message': f'{name} is not properly registered as a student. Please contact administrator.'
                        }
                    else:
                        # User is Faculty or other role
                        print(f"❌ {name} (ID: {User_ID}) is a {role}, not a student. Cannot mark attendance.")
                        return {
                            'success': False, 
                            'message': f'{name} is a {role}. Only students can mark attendance.'
                        }
                
                print(f"✅ {name} (ID: {User_ID}) is a student. Proceeding to mark attendance...")
                ...
                return {
                    'success': False,
                    'message': f'Error verifying student status: {str(e)}'
                }
        else:
    ...
    def mark_attendance(self, User_ID, Name, Session_ID, Subject_ID):
        ...
        # Double-check user is a student before inserting (safety check)
        try:
            student_check = "SELECT User_ID FROM Students WHERE User_ID = %s"
            self.cursor.execute(student_check, (User_ID,))
            is_student = self.cursor.fetchone()
            ...
            if not is_student:
                ...
                if role == 'Student':
                    # User has Student role but not in Students table - data inconsistency
                    print(f"❌ Validation failed: {Name} (ID: {User_ID}) has Student role but is not registered in Students table.")
                    return False, f"{Name} is not properly registered as a student. Please contact administrator."
                else:
                    # User is Faculty or other role
                    print(f"❌ Validation failed: {Name} (ID: {User_ID}) is a {role}, not a student.")
                    return False, f"{Name} is a {role}. Only students can mark attendance."
            
            print(f"✅ Validation passed: {Name} (ID: {User_ID}) is a student.")
        except Exception as e:
            print(f"❌ Error in student validation: {e}")
            return False, f"Error validating student status: {str(e)}"
        ...
        try:
            ...
            if "1452" in error_msg and "Students" in error_msg:
                return False, f"{Name} is not a student. Only students can mark attendance."
